...
#if ! defined( STL_SPLIT_JOIN_WCHAR_HEADER_FILE )

#define STL_SPLIT_JOIN_WCHAR_HEADER_FILE
...
inline void split(_In_z_ wchar_t const* source, _In_z_ wchar_t const * field_delimiter, _Inout_ std::vector<std::wstring>& string_array) noexcept
{
    ...
    auto const terminator_length{ std::char_traits<wchar_t>::length(field_delimiter) };
    ...
    {
        if (_wcsnicmp(&source[source_index], field_delimiter, terminator_length) == I_AM_EQUAL_TO_THAT)
        {
...
inline void split(_In_z_ wchar_t const* source, _In_ wchar_t const field_delimiter, _Inout_ std::vector<std::wstring>& string_array) noexcept
{
    ...
    split(source, std::char_traits<wchar_t>::length(source), field_delimiter, string_array);
}
...
#endif // STL_SPLIT_JOIN_WCHAR_HEADER_FILE